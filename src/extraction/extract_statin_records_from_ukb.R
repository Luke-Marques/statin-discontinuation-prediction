...
## singularity run --bind /scratch/users/k1763489/Documents/Projects/StatinRxRecords:/study,/scratch/prj/premandm/:/ukbiobank docker://onekenken/ukbkings:0.2.3
...
# read ehr codelist rds file from https://github.com/philipdarke/ehr-codesets
ehr_codes_url <- "https://github.com/philipdarke/ehr-codesets/raw/main/drugs/prescriptions.rds"
ehr_codes <- readRDS(url(ehr_codes_url))
# NOTE: can create your own rds file for other drug classes
...
bnf_pattern <- "^0212"
read_pattern <- paste0("^", ehr_codes$statins$all$read, collapse="|")
name_pattern <- paste0(ehr_codes$statins$all$search, collapse="|")
...
out_path = "/ukbiobank/usr/luke/rx_data/statins_rx_records.parquet"
write_parquet(rx_statins, out_path)

# create symlink to parquet file in study directory
link_path = "/study/data/statins/statins_raw.parquet"
utils::createLink(
    link=link_path,
    target=out_path
)
